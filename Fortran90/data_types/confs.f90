	subroutine confs(state,l)
        ...
        use approx_inst
	implicit none
	integer, dimension(2**obj_approx%nsites) :: state
	integer, allocatable, dimension(:,:) :: states
	integer i, j, k, l, f
	save states

	if(l.eq.0) then
         allocate(states(2**obj_approx%nsites,obj_approx%nsites))
	 do i=1,2**obj_approx%nsites
