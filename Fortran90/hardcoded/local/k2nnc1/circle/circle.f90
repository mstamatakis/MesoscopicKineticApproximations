	program kikuchi_2nn_c1
	implicit none
	integer i
	integer, dimension(13) :: state
	integer, allocatable, dimension(:) :: vec
	real*8 h, j, hamiltonian, part, corr, test
	...
	x=0.d0
	call newton(h,j,x,2,chemp,temp)
	write(*,*) (x(i), i=1,4)	
	...
	do i=1,p
	 do k=1,p
	  ...
	  jac(i,k)=(test(i,p,xfwd)-test(i,p,xbwd))/(2.d0*x(k)*d)
	 end do
	end do
	end subroutine
	...
	do i=2,7
	 hamiltonian=hamiltonian+(h+x(1))*state(i)+j*state(i)*state(1)
        ...
        do i=8,13
         hamiltonian=hamiltonian+(h+x(2))*state(i)
        end do
	do i=2,6
	 hamiltonian=hamiltonian+(j+x(3))*state(i)*state(i+1)
	end do
	hamiltonian=hamiltonian+(j+x(3))*state(2)*state(7)
        do i=8,12
         hamiltonian=hamiltonian+(j+x(4))*state(i)*(state(i-5)+state(i-6))
        end do	
	hamiltonian=hamiltonian+(j+x(4))*state(13)*state(7)
	hamiltonian=hamiltonian+(j+x(4))*state(13)*state(2)
	end function
