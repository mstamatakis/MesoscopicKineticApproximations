	...
	do i=1,cls
	 v1=i
	 r=r+corr(v1,1,lev,cls,h,j,g,x,y,latcon,chemp,temp)
	end do
	r=r/part(lev,cls,h,j,g,x,y,latcon,chemp,temp)
	r=r/cls
	write(*,*) r
	end program

	subroutine bisec(v1,v2,n1,n2,lev,cls,h,j,g,x,y,latcon,chemp,temp)
	! Bisection method to solve for g
	implicit none
	...
	gb=3.d0
	g=0.d0
        fa=func(v1,v2,n1,n2,lev,cls,h,j,ga,x,y,latcon,chemp,temp)
	fb=func(v1,v2,n1,n2,lev,cls,h,j,gb,x,y,latcon,chemp,temp)	
	if((fa.lt.0.d0).and.(fb.gt.0.d0)) then
	 i=0
	 do while(i.ne.1000)
	  i=i+1
	  gc=(gb+ga)/2.d0
	  fc=func(v1,v2,n1,n2,lev,cls,h,j,gc,x,y,latcon,chemp,temp)
	...
	do i=1,cls
	 ...
	 if(ri-latcon*(lev-1.d0).le.0.001d0) then
	  do k=1,cls
	   rk=sqrt(x(k)*x(k)+y(k)*y(k))
	   if(abs(rk-latcon*(lev-1.d0)).le.0.001d0) then
	    if((abs(sqrt((x(i)-x(k))**2+(y(i)-y(k))**2)-latcon).le.0.01d0).and.(i.ne.k)) then
	...
	real*8 function part(lev,cls,h,j,g,x,y,latcon,chemp,temp)
	...
	part=0.d0
        ...
        real*8 function corr(vec,n,lev,cls,h,j,g,x,y,latcon,chemp,temp)
        implicit none
	...
	real*8 function func(v1,v2,n1,n2,lev,cls,h,j,g,x,y,latcon,chemp,temp)
	implicit none
        integer i, k, cls, lev, s, n1, n2
        real*8 latcon, h, j, g, tot
        real*8, dimension(cls) :: x, y
        integer, dimension(n1) :: v1
	integer, dimension(n2) :: v2
	...
	func=log(corr(v1,n1,lev,cls,h,j,g,x,y,latcon,chemp,temp))-log(corr(v2,n2,lev,cls,h,j,g,x,y,latcon,chemp,temp))
	end function
