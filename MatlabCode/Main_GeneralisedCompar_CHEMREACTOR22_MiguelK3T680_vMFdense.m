...
%[0.1165    0.2745    0.3094    0.0524   -0.0310    0.1431   -0.0441    0.0694   -0.1377   -0.1548]; % guess values for parameters (row vector)
H0(kAppr) = 1.0713; % Hamiltonian's constant (for renormalisations)
...
%[0.1730    0.2695    0.1752   -0.0576   -0.0223   -0.0027    0.0029   -0.0080   -0.1360   -0.0806   -0.0579   -0.0771    0.1049]; % guess values for parameters (row vector)
...
logyNO2yNOrangeMF = logyNO2yNOrange(1):0.01:logyNO2yNOrange(end);
...
for i = 1:length(logyNO2yNOrangeMF);
        
    logyNO2yNO = logyNO2yNOrangeMF(i);
    ...
    % molecular NO2 and NO ideal gases at 1 atm
    muOstar = kboltz*Temp*logyNO2yNO + Go_NO2(Temp) - Go_NO(Temp);
    mOstarRangeMF = [mOstarRangeMF muOstar];
    ...
    for k = 1
        ...
        [PFcn,gcsoln,STheta,SCorrel,SCorrelUnocc,...
            ...
            muOstar,h,nn,ZPEO,QvibO,ApproxName{k},...
            CorrelLHS{k},CorrelRHS{k},...
            nsites{k},gcsoln,options);

        ThetaMF(i) = STheta; % coverage
        CorrelOccupMF(i) = SCorrel; % correlation of occupied sites
        CorrelUnoccMF(i) = SCorrelUnocc; % correlation of unoccupied sites
        AvgekO2adsMF(i) = SAvgekO2ads;
        AvgekO2desMF(i) = SAvgekO2des;

    end
    
end

RO2adsMF = 2*nn*ApreO2ads*yO2*Pres*AvgekO2adsMF.*CorrelUnoccMF;
RO2desMF = 2*nn*ApreO2ads/PERatioO2adsdes*AvgekO2desMF.*CorrelOccupMF;
NetRateMF = RO2adsMF-RO2desMF;
indicMF = sign(NetRateMF);
indicFwdMF = indicMF > 0;
indicRevMF = indicMF < 0;
...
load('Main_GeneralisedCompar_CHEMREACTOR22_MiguelK3T680_vMFdense.mat')
...
if compareMC
 ...
 %       plot(logconvc*0.2,7.949052e-01,'>','Color',[0 0 0],'MarkerFaceColor','None','MarkerSize',6)
    plot(logconvc*0.2,-1.013837e+00,'^','Color',[200 100 0]/255,'MarkerFaceColor','None','MarkerSize',6)
    plot(logconvc*0.2,-1.811077e+00,'o','Color',[255 200 0]/255,'MarkerFaceColor','None','MarkerSize',6)
    NetRateGCMC = exp(M(:,2).');
...
k = 1
hplot = [hplot plot(logconvc*logyNO2yNOrangeMF(indicFwdMF(:)),logconvc*log(NetRateMF(indicFwdMF(:))),...
    [ApproxLinSpec{k} ApproxMarkr{k}],'Color',ApproxColor{k},...
    'MarkerFaceColor',ApproxColor{k},'MarkerSize',ApproxMSize(k))];
plot(logconvc*logyNO2yNOrangeMF(indicRevMF(:)),logconvc*log(-NetRateMF(indicRevMF(:))),...
    [ApproxLinSpec{k} ApproxMarkr{k}],'Color',ApproxColor{k},...
...
text(-1.8,5.9,'(f)','FontSize',30) 
%text(4.342945e-02,5.372262e-01,'>','FontSize',12)
%text(4.342945e-02,-1.253991e+00,'>','FontSize',12)
%text(4.342945e-02,-2.126004e+00,'>','FontSize',12)
eval(['print -dpng TOFvsRatioK3T680']);
eval(['print -depsc fig5f']);
...
text(-1.8,1.1,'(f)','FontSize',30)
eval(['print -dpng ThetaVsNO2NORatioK3T680']);
eval(['print -depsc ThetaVsNO2NORatioK3T680']);
