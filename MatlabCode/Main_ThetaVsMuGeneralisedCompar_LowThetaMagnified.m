...
if compareMC
    if Temp == 480
%         M = load('E:\Academic\Research\CEng_Lectureship_UCL\Computations\ReducedKineticModels\MetropolisLatticeGasFortranRestartable\OutputLegion\MiguelSuspicious20170531\results_output.txt');
%         M = load('E:\Academic\Research\CEng_Lectureship_UCL\Computations\ReducedKineticModels\MetropolisLatticeGasFortranRestartable\OutputLegion\Run_096x072_FullRange_T480_From_m1.5\results_output.txt');
%         M = load('results_output.txt');
%         M = load('results_output_extended.txt');
        M = load('results_output_MiguelNewAppendOriginal.txt');
    elseif Temp == 680
...
text(-1.78,1.09,'(a)','FontSize',30)
eval(['print -dpng ThetaVsMuBP']);
eval(['print -depsc Fig4a']);
...
text(-0.125,1.09,'(d)','FontSize',30)
eval(['print -dpng CorrelVsThetaBP']);
eval(['print -depsc Fig4d']);
