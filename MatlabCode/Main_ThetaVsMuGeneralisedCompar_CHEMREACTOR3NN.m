...
gcguess{kAppr} = 1*0.2332; % guess values for parameters (row vector)
H0(kAppr) = 1*0.3016; % Hamiltonian's constant (for renormalisations)
...
CorrelLHS{kAppr} = {1}; % left hand side in correlation equations
CorrelRHS{kAppr} = {2}; % right hand side in correlation equations
gcguess{kAppr} = 1*0.1803; % guess values for parameters (row vector)
H0(kAppr) = 1*0.3147; % Hamiltonian's constant (for renormalisations)
...
gcguess{kAppr} = 1*[0.1805   -0.0650]; % guess values for parameters (row vector)
H0(kAppr) = 1*0.3147; % Hamiltonian's constant (for renormalisations)
...
gcguess{kAppr} = 0*[0.1165    0.2745    0.3094    0.0524   -0.0310    0.1431   -0.0441    0.0694   -0.1377   -0.1548]; % guess values for parameters (row vector)
H0(kAppr) = 0*1.0713; % Hamiltonian's constant (for renormalisations)
ApproxColor{kAppr} = [200 200 0]/255; % FOR PLOTTING: color
ApproxMarkr{kAppr} = 'h'; % FOR PLOTTING: marker
ApproxLinSpec{kAppr} = '-'; % FOR PLOTTING: line type
ApproxMSize(kAppr) = 8; % FOR PLOTTING: marker size
...
ApproxMarkr{kAppr} = 'o'; % FOR PLOTTING: marker
ApproxLinSpec{kAppr} = '-'; % FOR PLOTTING: line type
ApproxMSize(kAppr) = 5; % FOR PLOTTING: marker size
end
if 0
% % Next the K3NN approximation
 ...
 gcguess{kAppr} = 0*[0.1165    0.2745    0.0524   -0.0310    0.1431]; % guess values for parameters (row vector)
...
if 1
 % % Next the K3NN approximation 1 correction parameter
 kAppr = kAppr + 1; % index of the approximation
 ApproxName{kAppr} = 'K3NN1corr'; % name of the approximation
 nsites{kAppr} = 19; % number of sites in the approximation's cluster
 CorrelLHS{kAppr} = {1}; % left hand side in correlation equations
 CorrelRHS{kAppr} = {14}; % right hand side in correlation equations
 gcguess{kAppr} = 0*0.1431; % guess values for parameters (row vector)
 H0(kAppr) = 1.0713; % Hamiltonian's constant (for renormalisations)
 ApproxColor{kAppr} = [200 100 0]/255; % FOR PLOTTING: color
 ApproxMarkr{kAppr} = '^'; % FOR PLOTTING: marker
 ApproxLinSpec{kAppr} = '-'; % FOR PLOTTING: line type
...
if 0
 % % Next the K3NN approximation 1 correction parameter
 kAppr = kAppr + 1; % index of the approximation
 ApproxName{kAppr} = 'K3NN2corr'; % name of the approximation
 nsites{kAppr} = 19; % number of sites in the approximation's cluster
 CorrelLHS{kAppr} = {1, 1}; % left hand side in correlation equations
 CorrelRHS{kAppr} = {8,14}; % right hand side in correlation equations
 gcguess{kAppr} = 0*[0.1431   0.1165]; % guess values for parameters (row vector)
 H0(kAppr) = 1.0713; % Hamiltonian's constant (for renormalisations)
 ApproxColor{kAppr} = [200 100 0]/255; % FOR PLOTTING: color
 ApproxMarkr{kAppr} = '^'; % FOR PLOTTING: marker
 ApproxLinSpec{kAppr} = '-'; % FOR PLOTTING: line type
...
if compareMC
    hplot = [hplot plot(M(1:2*Di:end,1),M(1:2*Di:end,2),'-.sr','MarkerFaceColor','r','MarkerSize',4)];
...
%eval(['print -dpng ThetaVsMu3NN2corr']);
%eval(['print -depsc ThetaVsMu3NN2corr']);

eval(['print -dpng ThetaVsMu3NN1corr']);
eval(['print -depsc ThetaVsMu3NN1corr']);
...
if compareMC
    hplot = [hplot plot(M(1:Di:end,2),M(1:Di:end,4),'-.sr','MarkerFaceColor','r','MarkerSize',4)];
    legendprop = {legendprop{:} 'MMC'};
end
...
%eval(['print -dpng CorrelVsTheta3NN2corr']);
%eval(['print -depsc CorrelVsTheta3NN2corr']);

eval(['print -dpng CorrelVsTheta3NN1corr']);
eval(['print -depsc CorrelVsTheta3NN1corr']);
