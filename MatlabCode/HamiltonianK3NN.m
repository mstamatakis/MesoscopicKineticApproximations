function out1 = HamiltonianK3NN(H0,H1,h,nn,g1,g2,c1,c2,c3,epsl)
...
for i = 8:12
    ...
    EdgeInter12 = EdgeInter12 + ...
        (h+c1)*epsl(:,i).*(epsl(:,ineigh1) + epsl(:,ineigh2));
end
EdgeInter12 = EdgeInter12 + ...
    (h+c1)*epsl(:,13).*(epsl(:,7) + epsl(:,2)); % 1NN-2NN 2-body terms

EdgeInter13 = 0;
for i = 14:19
    ineigh1 = i-12;
    EdgeInter13 = EdgeInter13 + (h+c2)*epsl(:,i).*epsl(:,ineigh1);
...
for i = 15:19
    ...
    EdgeInter23 = EdgeInter23 + ...
        (h+c3)*epsl(:,i).*(epsl(:,ineigh1) + epsl(:,ineigh2));
end 
EdgeInter23 = EdgeInter23 + ...
    (h+c3)*epsl(:,14).*(epsl(:,13) + epsl(:,8)); % 2NN-3NN 2-body terms
