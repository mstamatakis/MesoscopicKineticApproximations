...
gcguess{kAppr} = 1*[0.0669    0.2188   -0.0084   -0.0380]; % guess values for parameters (row vector)
H0(kAppr) = 1*0.6568; % Hamiltonian's constant (for renormalisations)
...
gcguess{kAppr} = 1*[0.0866    0.3923    0.0723    0.0012   -0.0442   -0.1952]; % guess values for parameters (row vector)
H0(kAppr) = 1*0.7100; % Hamiltonian's constant (for renormalisations)
...
gcguess{kAppr} = 1*[0.0956    0.3970    0.0604   -0.0036   -0.0474   -0.1970   -0.0063]; % guess values for parameters (row vector)
H0(kAppr) = 1*0.6927; % Hamiltonian's constant (for renormalisations)
ApproxColor{kAppr} = [255 200 0]/255; % FOR PLOTTING: color
...
gcguess{kAppr} = 1*[0.1730    0.2695    0.1752   -0.0576   -0.0223   -0.0027    0.0029   -0.0080   -0.1360   -0.0806   -0.0579   -0.0771    0.1049]; % guess values for parameters (row vector)
H0(kAppr) = 1*1.0915; % Hamiltonian's constant (for renormalisations)
ApproxColor{kAppr} = [255 200 0]/255; % FOR PLOTTING: color
ApproxMarkr{kAppr} = 'o'; % FOR PLOTTING: marker
ApproxLinSpec{kAppr} = '-'; % FOR PLOTTING: line type
...
%text(-1.715,1.09,'(c)','FontSize',30)
text(-1.78,1.09,'(b)','FontSize',30)
eval(['print -dpng ThetaVsMuK2']);
eval(['print -depsc Fig4b']);
...
%text(-0.1,1.09,'(d)','FontSize',30)
text(-0.125,1.09,'(e)','FontSize',30)
eval(['print -dpng CorrelVsThetaK2']);
eval(['print -depsc Fig4e']);
