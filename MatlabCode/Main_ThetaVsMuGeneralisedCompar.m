...
if compareMC
    hplot = [hplot plot(M(:,1),M(:,2),'-.sr','MarkerFaceColor','r','MarkerSize',8)];
    legendprop = {legendprop{:} 'Kinetic MC'};
end

for k = 1:kAppr
    hplot = [hplot plot(muOstarRange,Theta(k,:),...
...
if compareMC
    hplot = [hplot plot(M(:,2),M(:,4),'-.sr','MarkerFaceColor','r','MarkerSize',8)];
    legendprop = {legendprop{:} 'Kinetic MC'};
end

for k = 1:kAppr
    hplot = [hplot plot(Theta(k,:),CorrelOccup(k,:),...
        [ApproxLinSpec{k} ApproxMarkr{k}],'Color',ApproxColor{k},...
        'MarkerFaceColor',ApproxColor{k},'MarkerSize',ApproxMSize(k))];
    legendprop = {legendprop{:} ApproxName{k}};
end
