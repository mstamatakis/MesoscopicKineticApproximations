...
for k = 1:kAppr
    hplot = [hplot plot(logyNO2yNOrange(indicFwd(k,:)),log(NetRate(k,indicFwd(k,:))),...
        [ApproxLinSpec{k} ApproxMarkr{k}],'Color',ApproxColor{k},...
        'MarkerFaceColor',ApproxColor{k},'MarkerSize',ApproxMSize(k))];
    plot(logyNO2yNOrange(indicRev(k,:)),log(-NetRate(k,indicRev(k,:))),...
        [ApproxLinSpec{k} ApproxMarkr{k}],'Color',ApproxColor{k},...
...
xlabel('ln(P_{NO2}/P_{NO})',...
    'FontName',FntNm,...
    'FontWeight','Bold',...
    'FontSize',FntSz+6);

ylabel('ln(TOF) [TOF in s^{-1}]',...
    'FontName',FntNm,...
    'FontWeight','Bold',...
    'FontSize',FntSz+6);
if ~isempty(hplot)
...
if compareMC
    indicFwdKMC = M(:,3) > 0;
    indicRevKMC = M(:,3) < 0;
    hplot = [hplot plot(M(indicFwdKMC,1),M(indicFwdKMC,5),'-.sr','MarkerFaceColor','r','MarkerSize',8)];
    plot(M(indicRevKMC,1),M(indicRevKMC,5),'-.sr','MarkerSize',8)
    legendprop = {legendprop{:} 'Kinetic MC'};
end

for k = 1:kAppr
    hplot = [hplot plot(logyNO2yNOrange(indicFwd(k,:)),Theta(k,indicFwd(k,:)),...
        [ApproxLinSpec{k} ApproxMarkr{k}],'Color',ApproxColor{k},...
        'MarkerFaceColor',ApproxColor{k},'MarkerSize',ApproxMSize(k))];
    plot(logyNO2yNOrange(indicRev(k,:)),Theta(k,indicRev(k,:)),...
        [ApproxLinSpec{k} ApproxMarkr{k}],'Color',ApproxColor{k},...
        'MarkerFaceColor','None','MarkerSize',ApproxMSize(k))
    legendprop = {legendprop{:} ApproxName{k}};
end
