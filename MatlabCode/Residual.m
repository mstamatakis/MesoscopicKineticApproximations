function outResid = Residual(ApproxIdent,CorrelLHS,CorrelRHS,nsites,beta,...
    H0,H1,h,nn,mu,HamiltPars)
...
if strcmp(ApproxIdent,'MF')
    if nEq == 0 % this is valid only for the MF approximation
        ...
        outResid = z - 1/(1+exp(beta*(H1-mu+h*nn*z)));
        return
    else
...
[PFcn,PFcnEps1_Z,PFcnEps0_Z,PFcnPartial] = ...
    ...
    {CorrelLHS{:},CorrelRHS{:}},...
    nsites,beta,H0,H1,h,nn,mu,HamiltParsCell{:});
