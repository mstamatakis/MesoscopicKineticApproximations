subroutine approx_initialise_1NN(obj_approx)
	...
	class (approximation) :: obj_approx
        ...
        interface
           subroutine  sigma1_sigma2_1NN(obj_approx)
        ...
        interface
           subroutine  sigma1sigma2_sigma2sigma3_1NN(obj_approx)
               import approximation
               class(approximation) :: obj_approx
           end subroutine
        end interface
	...
	allocate(obj_approx%hamilt%internbody(nterms),source = 0) ! number of bodies for each interaction term
        ...
        do i = 1, norig
            obj_approx%hamilt%origpars(i) = int_values(i) ! values of an original interaction term in the Hamiltonian
        enddo
	...
	allocate(obj_approx%hamilt%corcterms(nterms),source = 0) ! will be used as a pointer to a correction interaction term in the Hamiltonian
        ...
        if (cal_parser%get_cluster_depth().eq.'1NN') then ! for the momment it is not really needed
	...
	! Integer arrays to be used as pointers to terms and corrections:
           ...
           do i = 1,cal_parser%get_ncorrterms()
               if ((cor_names(i).eq.'SB').or.(cor_names(i).eq.'SBWI')) then   
                   obj_approx%hamilt%corcterms(2:nsites) = 1   
               elseif (cor_names(i).eq.'1NN') then 
                   obj_approx%hamilt%corcterms(nsites+7:nterms) = 2
               endif
           enddo
	   ...
	   allocate(obj_approx%eqns%corrlnbody(obj_approx%eqns%nterms),source = 0)
           ...
           if (ncorc.eq.1) then !SB and SBWI
               call sigma1_sigma2_1NN(obj_approx)
           elseif (ncorc.eq.2) then !SB plus 1NN
               call sigma1_sigma2_1NN(obj_approx)
               call sigma1sigma2_sigma2sigma3_1NN(obj_approx)
           endif
...
end subroutine approx_initialise_1NN
...
subroutine sigma1_sigma2_1NN(obj_approx)
        ...
        return          
end subroutine sigma1_sigma2_1NN
...
subroutine sigma1sigma2_sigma2sigma3_1NN(obj_approx)
        ...
        return          
end subroutine sigma1sigma2_sigma2sigma3_1NN 
